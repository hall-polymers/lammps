...
void PairBornsolv::compute_inner()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r4inv,forcelj,factor_lj,rsw;
  int *ilist,*jlist,*numneigh,**firstneigh;
...
void PairBornsolv::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r4inv,forcelj,factor_lj,rsw;
...
void PairBornsolv::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r2inv,r4inv,forcelj,factor_lj,rsw;
...
void PairBornsolv::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairBornsolv::init_one(int i, int j)
{
  ...
  lj1[i][j] = -1.0 * epsilon[i][j] * pow(sigma[i][j],4.0);
  lj2[i][j] = -4.0 * epsilon[i][j] * pow(sigma[i][j],4.0);
